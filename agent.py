...
class Agent():
    def __init__(self, id, args, train_dataset=None, data_idxs=None):
        ...
        if train_dataset is None:
            print('train_dataset is none, load Fed_EMNIST') # for testing
            ...
            if self.id < args.num_corrupt:
                utils.poison_dataset_backdoor(self.train_dataset, args, data_idxs, agent_idx=self.id)
        else:
            self.train_dataset = utils.DatasetSplit(train_dataset, data_idxs)
            # for backdoor attack, agent poisons his local dataset
            if self.id < args.num_corrupt:
                utils.poison_dataset_backdoor(train_dataset, args, data_idxs, agent_idx=self.id)
